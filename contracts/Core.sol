...
contract Core is Ownable, Pausable, ReentrancyGuard {
    ...
    /**
     * @dev Mint country tokens to Core (for initial supply)
     * This allows Core to mint tokens from FlagWarsToken contracts
     */
    function mintCountryTokens(uint256 id, uint256 amount18) external onlyOwner {
        require(countries[id].exists, "Core: country not found");
        require(amount18 > 0 && amount18 % 1e18 == 0, "Core: invalid amount");
        
        // Call token.mint() - token contract will verify msg.sender == core
        // We need to use a low-level call since we don't have the token ABI here
        (bool success, ) = countries[id].token.call(
            abi.encodeWithSignature("mint(address,uint256)", address(this), amount18)
        );
        require(success, "Core: mint failed");
    }
