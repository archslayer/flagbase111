...
const processor = makeWorker<AttackJob>('attack-events', async ({ data, id }) => {
  ...
  try {
    ...
    const consumeResult = await processAttackEvent({
      wallet: data.user,
      txHash: data.txHash,
      feeUSDC6: data.feeUSDC6,
      timestamp: data.timestamp
    })
    if (consumeResult.consumed) {
      console.log(`[Q:attack-events] consumed free attack for ${userLower}`)
    } else if (consumeResult.reason === 'no-free-left') {
      console.warn(`[Q:attack-events] no free attack available for ${userLower}, tx=${data.txHash}`)
    }
