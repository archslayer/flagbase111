...
import { z } from 'zod'
import { getSellQuote, assertFresh } from '@/lib/quotes'
...
async function sellHandler(req: NextRequest): Promise<NextResponse> {
  try {
  ...
  try {
    ...
    const quote = await getSellQuote(countryId, amountToken18)
    assertFresh(quote)

    // UI'dan gelen parametreleri kullan, yoksa fallback
    const minOut = uiMinOut ?? quote.minOut
    ...
    const hash = await writeWithTimeout(
      wc.writeContract({
        ...
        abi: CORE_ABI,
        functionName: 'sell',
        args: [BigInt(countryId), amountToken18, minOut, deadline],
      })
    )
    ...
    queueMicrotask(async () => {
      try {
        ...
        queueMicrotask(() => incrementTxStatus(status, 'sell'))
        ...
        queueMicrotask(() => incrementTxStatus('FAILED', 'sell'))
      }
    })
    ...
    return NextResponse.json({ ok: false, code: 'SELL_FAILED', error: userMsg, txId }, { status })
  } finally {
    release()
  }
  } catch (e: any) {
    ...
    return NextResponse.json({ 
      ok: false, 
      code: 'INTERNAL',
      message: e?.message || 'SELL_FAILED' 
    }, { status: 500 })
  }
}

async function sellRouteHandler(req: NextRequest): Promise<NextResponse> {
...
export const POST = sellRouteHandler
